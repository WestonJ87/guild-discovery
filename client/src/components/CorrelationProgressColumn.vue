<template>
  <div class="sizing-container">
        <div class="radial-progress" v-bind:data-progress="params.value">
            <div class="circle">
                <div class="mask full">
                    <div class="fill"></div>
                </div>
...
export default {
  name: 'CorrelationProgressColumn',
  data () {
    return {
      name: 'CorrelationProgressColumn',
      value: 0
...
.radial-progress {
	...
	@for $i from 0 through 100 {
		@if $i < 40 {
			$circle-color: #a7391d;
			$percentage-color: #a7391d;
		} @else if $i < 72 {
			$circle-color: #c0b51f;
			$percentage-color: #c0b51f;
		} @else if $i >= 72 {
			$circle-color: #97a71d;
			$percentage-color: #97a71d;
		} @else {
			@error "Unknown value #{$i}.";
		}
		...
		&[data-progress="#{$i}"] {
			...
			.inset .percentage .numbers {
                margin-top: -$i * $percentage-font-size + $percentage-font-size;
			}
		}
	}
}
